...
(function () {
  ...
  if (!isMF) {
    ...
    if (!req || Date.now() - req.ts > 10 * 60 * 1000) return; // リクエストがない/古い
    ...
    (async () => {
      ...
      for (let i = 0; i < req.months.length; i++) {
        ...
        // ページ内の実際のCSVダウンロードリンクをベースに年月だけ差し替え（仕様変更に強い）
        ...
        await new Promise((r) => setTimeout(r, 700)); // 連続アクセスを避ける
      }
      ...
      banner.textContent = errors.length
        ? `⚠ 一部失敗: ${errors.join(' / ')}（成功 ${items.length}件は反映されます）`
        : `✅ ${items.length}ヶ月分を取得しました。アプリのタブに戻ってください（このタブは閉じてOK）`;
    })();
  }
})();
